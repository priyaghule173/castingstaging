  ...
  <div class="col-lg-8">
   <div class="data_card">
     ...
     <ul class="nav nav-tabs">
      <?php
      $Pending = $this->db->select('count(d_id) as total')->from('directors')->where('status','0')->get()->result_array();
      ?>
      <li class="active"><a data-toggle="tab" href="#tab1">Pending Directors Approval&nbsp;(<?php echo $Pending[0]['total']?>)</a></li>
      <?php
      $Approved = $this->db->select('count(d_id) as total')->from('directors')->where('status','1')->get()->result_array();
      ?>
      <li><a data-toggle="tab" href="#tab2">Active/ Approved Directors&nbsp;(<?php echo $Approved[0]['total']?>)</a></li>
      <?php
      $Rejected = $this->db->select('count(d_id) as total')->from('directors')->where('status','2')->get()->result_array();
      ?>
      <li><a data-toggle="tab" href="#tab3">Rejected Directors&nbsp;(<?php echo $Rejected[0]['total']?>)</a>
      </li>
    </ul>
            ...
            <tbody style="font-size: 12px;">
              ...
              foreach ($result as $row) {
                ...
                <tr>
                  ...
                  <td>
                    <select name="emp_active" onchange="showUser(this.value,<?= $row['d_id']?>)">
...
function showUser(str,DID)
{
  var DID_new = DID;
  var dir_action = str;
  $.ajax({
    type: "POST",
    url: "<?php echo base_url('admin/update_director_status')?>",
    data: {'d_id': DID_new ,'status' :dir_action},
    success: function () {
      alert("Status Updated Successfully!..");
      window.location.href = "<?php echo base_url('admin/manage_directors')?>";     
    }
  });
}
</script>
