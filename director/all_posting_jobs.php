                         ...
                         <div class="row">
                             <div class="media">
                               <div class="media-left">
                                 <img  height="120" width="120" src="<?php echo base_url()?>assets/images/profile_pic/<?php echo $profile->profile_pic; ?>">
 
                                 </div>
                                 <div class="media-body">
                                    <h3 class="text-left">
                                      <a  href="<?php echo base_url('director/job_details?job_id=').$key['job_id']?>" target="_blank" >
                                      <?php echo $key['job_title'];?><a  target="_blank" href="<?php echo base_url('director/view_job_applications?job_id=').$key['job_id'] ?>"><span class="badget-text">Job Applications: <span class="badge"><?php echo $count[0]['total'];?></span></span></a>
                                    </a>
                                  </h3>
                                
                                 <p><?php echo substr($key['job_desc'],0,150)." "; ?>

                                    <a  target = "_blank" style="text-decoration:underline" href="<?php echo $key['job_id']; ?>"> Read More</a>
                                  </p>
                                 <h4>Age Group :<?php echo $key['age_from']." to ".$key['age_upto']." Years";?></h4>
                                <?php 
                                $e = $key['job_post_expired_date'];
                                $future = strtotime($e); //Future date.
                                $s = date("Y-m-d"); //source time
                                $timefromdb = strtotime($s);
                                $timeleft = $future - $timefromdb;
                                $daysleft = round((($timeleft/24)/60)/60);
                          
                                ?>
                                 <h4><?php if($daysleft >= "0" ){
                                  ?><?php echo $daysleft."Days Remaining "; }
                                  else{
                                    ?>Job expired :<?php echo abs($daysleft)." Day Ago"; } ?>
                                </h4>
                                <h4 class="highlightgreen-text pull-right1"><a style="color:#fff;"  href="<?php echo base_url('director/edit_job_post_request?job_id='.$key['job_id'])?>"> Edit Job</a>

                                 <h4 class="highlightgreen-text pull-right"> Job status : <?php  if($key['job_status']=="0"){
                                  echo " Pending";
